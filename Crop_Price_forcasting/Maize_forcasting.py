...
class Maize_forcasting:
    ...
    def Train(self):
        ...
        holidays = pd.concat((Drought, Floods))

        # instantiate the model and fit the timeseries
        model = Prophet(interval_width=.95,changepoint_range=1,holidays=holidays,seasonality_mode='multiplicative')
        model.fit(train)

        # save the model to disk
        self.filename = 'Maize_forcasting.sav'
